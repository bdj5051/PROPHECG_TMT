...
class CustomDataset(Dataset):
    def __init__(self, s3_path, transform: Union[Compose, None] = None):
        # S3 경로에서 Parquet 데이터셋 로드
        self.transform = transform,
        self.dataset = pq.ParquetDataset(s3_path).read().to_pandas()["fdata"]
    ...
    def __getitem__(self, idx):
        ...
        if len(data[-1]) <300:
            num_copies = 300 - len(data[-1])
            extended_arrays = [np.append(arr, np.full(num_copies, arr[-1])) for arr in data]
            trace = np.array(extended_arrays)
            
            trace = torch.from_numpy(trace).type(torch.float)
            trace = trace * 0.00488
        else:
            checked_arrays = [arr[:300] for arr in data]
            trace = np.array(checked_arrays)
...
class UniversalECGDataset(DatasetBase):
    ...
    def loadUMCUSample(data_dir: str, pseudo_id: str, test_id: str):
        ...
        """
        x = pd.read_csv(os.path.join(data_dir, f'{test_id}.csv'))
        trace = {'I' : x['I'],
                'II' : x['II'],
                'III' : x['III'],
                'aVR' : x['aVR'],
        ...
        trace[['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']]=trace[['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']].astype(str).astype(int)
        
        # for TMT dataset
        if len(trace)<300:
            copy = trace[-1:]
            num_copies = 300-len(trace)
            repeated_rows = pd.concat([copy] * num_copies, ignore_index=True)
            trace = pd.concat([trace, repeated_rows], ignore_index=True)
        else:
            trace = trace[0:300]
        trace = trace.to_numpy()
        trace = np.transpose(trace)
        return trace, test_id
